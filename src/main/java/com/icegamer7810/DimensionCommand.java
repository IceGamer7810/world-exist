...
public final class DimensionCommand {
    ...
    public static LiteralCommandNode<CommandSourceStack> create(final WorldExistenceService worldExistenceService) {
        return Commands.literal("dimension")
            .then(Commands.literal("check")
                .then(Commands.argument("input", VanillaScoreHolderArgument.nativeType())
                    .executes(context -> {
                        final String input = extractRawInput(context);
    ...
    private static String extractRawInput(final com.mojang.brigadier.context.CommandContext<CommandSourceStack> context) {
        for (int i = context.getNodes().size() - 1; i >= 0; i--) {
            final ParsedCommandNode<CommandSourceStack> node = context.getNodes().get(i);
            if (!"input".equals(node.getNode().getName())) {
                continue;
            }
        ...
        throw new IllegalStateException("Missing input argument range");
    }
